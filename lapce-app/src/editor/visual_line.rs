...
//! However, editors require features like wrapping and multiline phantom text.  
//!   
//! When rendering with either of those, we have to display based on visual lines rather than the
...
//! default.  
//!  
//! There's two types of Visual Lines that we deal with.  
//! - [`VLine`]: Variables are often written with `vline`  
//! - [`RVLine`]: Variables are often written with `rvline`  
//!   
...
//! ----
//!  
//! A common trait that is used is the [`TextLayoutProvider`].  
//! This serves the dual purpose of giving us the text of the underlying file, as well as
//! constructing the text layouts that we use for rendering.   
//! Using this trait makes so rather than [`Lines`] keeping track of your text, you are simply
//! required to pass it in whenever the content is relevant. This avoids having to manually track
//! from outside when you need to update [`Lines`] with your new document.

// TODO: should we break this apart into multiple files? It's getting a bit long and will probably
// only grow.

// TODO(minor): We use a lot of `impl TextLayoutProvider`, partially because we really want the
// compiler to inline certain pieces of logic so it can do stuff smarter than a naive for-loop
// (though not sure how well it manages)
// but there's probably parts of it that overuse generics, or could unify under one text layout
// provider type. Ex: a lot of them take an `impl TextLayoutProvider` but have to call multiple
// functions and so pass an `&impl TextLayoutProvider`. We could just make the root take an `&impl`?
// Eh. Possibly just look at functions that don't benefit much from inlining, or for operations that
// could be moved onto text layout provider (impl'd automatically?) to benefit from the inlining.
...
/// The [`TextLayoutProvider`] provides us a [`Rope`] view into the text of whatever we are
/// wrapping.  
/// As well it provides us a way of constructing the text layout for a given line, given the font
/// size and wrapping status.  
///   
...
///  
/// Using this trait rather than passing around something like [`Document`] allows to swap out the
/// backend if needed. This would be useful if we ever wanted to reuse it across different views that did not naturally fit into the 'document' model. Or when we want to extract the editor view code int a separate crate for Floem.
pub trait TextLayoutProvider {
    ...
    // TODO: Do we really need to pass font size to this? The outer-api is providing line font size provider already, so it should be able to just use that.
    fn new_text_layout(
        &self,
        line: usize,
        font_size: usize,
...
pub enum LayoutEvent {
    ...
    // TODO: Should we emit this? Technically doc's cache rev changing captures I think almost all
    // of this?
    // Cleared,
}

pub struct Lines {
    /// This is inside out from the usual definition because we may want to swap out the font sizes
    /// in the future, while also grabbing an `Arc` to hold. Doing an `Arc<RefCell<_>>` has the
    /// issue that with a `dyn` it can't know they're the same size and we can't modify the outer
    ...
    // TODO(minor): Besides this we don't depend on reactive for the visual line logic, which is
    // probably better for isolating it. This could be changed to some generic trait with the
    // layout event on the outside.
...
impl Lines {
    ...
    /// - If there was no lines that exceeded the wrapping then we could do the fast path
    ...
    pub fn last_vline(&self, text_prov: impl TextLayoutProvider) -> VLine {
        // TODO: with this information we could cache the VLineInfo for it too, right?
        ...
        if let Some(last_vline) = self.last_vline.get() {
            ...
            // TODO: We could use rayon's join to do this in parallel
            // Though it might be overkill for reasonable small files, I'm unsure what the parallel
            // costs are.
    ...
    // TODO: test this. I'm not sure that find rel vline handles the length properly
    ...
    // TODO: Get rid of the clone bound. We can do that by creating a structure for this initialize
    // iterator that owns the `VLine`s as a subfield from which it can then just access the
    // text layout provider directly.
    ...
    pub fn vline_col_of_offset(
        ...
        // TODO: don't unwrap, just default to lastvlineinfo
        let vline_info = self.iter_vlines(&text_prov, false, vline).next().unwrap();
        ...
        let col = col.min(vline_info.last_col(&text_prov, true));

        (vline, col)
    }

    // TODO(minor): we could return a bool for whether it manages to actually definitely be on the specific visual line. Like, if the vline is entirely phantom then it would be false.
    ...
    pub fn vline_of_line(
        ...
        // TODO: we literally have a vline_of_line function, we should just use that instead of
        // converting to offset
        let offset = text_prov.rope_text().offset_of_line(line);

        let Some((vline, line_index)) =
            find_vline_of_offset(self, &text_prov, offset, CursorAffinity::Backward)
        else {
            // We assume it is out of bounds
            return self.last_vline(text_prov);
        };
    ...
    pub fn vline_of_rvline(
        ...
        if self.is_linear(&text_prov) {
            debug_assert_eq!(rvline.line_index, 0, "Got a nonzero line index despite being linear, this implies an old RVLine was used.");
    ...
    pub fn rvline_of_offset(
        ...
        let Some(rel) = find_rvline_of_offset(self, &text_prov, offset, affinity)
        else {
            // We assume it is out of bounds
            return self.last_rvline(text_prov);
        };
    ...
    // TODO: some callers could be replaced by one that uses their `VLineInfo<()>`
...
// TODO: does this work fine on the very last vline if it is empty?
// TODO: line column?
...
fn find_vline_of_offset(
    ...
    // TODO: We could work backwards if we have the last vline & we're in the latter half of the
    // file. It might be worth it to compute the last vline anyway, since so much uses it and it
    // would make future checks along this cheaper
...
// TODO: this should have affinity. For example, if you're getting the rvline of
// `text.len()` (or `text.len() - 1`?) then it matters where tha phantom text is!
...
// TODO: Is there some way to join this with the next_line_part to unify the logic? Also does this imply next line part is missing something?
// TODO: could return the vcolumn as well?
...
fn find_vline_init_info(
    ...
    // TODO(minor): We could probably some heuristic about whether to even calculate the last vline
    // (like, most files don't have more than two wrapped lines per line, so we could check if
    // we're past some threshold and then use the last vline if we are)
    let last_vline = lines.last_vline(&text_prov);
...
// TODO: should we package (VLine, buffer line) into a struct since we use it for these
    ...
    while cur_vline < vline.get() {
        ...
        let line_count = if let Some(text_layout) = layouts.get(font_size, cur_line)
        {
            ...
            if cur_vline + line_count > vline.get() {
                ...
                // TODO: is it fine to unwrap here?
                let (col, _) = text_layout
                    .layout_cols(&text_prov, cur_line)
                    .nth(line_index)
                    .unwrap();
...
fn vline_init_info_b(
    ...
    let rope_text = text_prov.rope_text();
    // TODO: is it fine to unwrap here? Ideally we should never get an invalid line by good code
    // but we really shouldn't panic if we do! Do a debug log instead.
    let (col, _) = text_layout
        .layout_cols(&text_prov, rv.line)
        .nth(rv.line_index)
        .unwrap();
...
fn prev_rvline(
    ...
    if line_index == 0 {
        ...
        if let Some(layout_line) = layouts.get(font_size, prev_line) {
            ...
            // TODO: don't unwrap
            let (i, (line_col, _)) = layout_line
                .layout_cols(&text_prov, prev_line)
                .enumerate()
                .last()
                .unwrap();
            let line_col =
...
mod tests {
    ...
    fn debug_breaks<'a>(
        ...
        let (text_prov, mut lines) = make_lines(text, width, true);
    ...
    // Various of these tests are adapted from xi's linewrap impl.
    // However, there are some modifications.
    // - Specific font size
    // - `\n`s are replaced with spaces in the underlying text layout logic, unlike xi's impl
    // - some tests now don't include the extra empty "" line at the end
    //   - TODO: is that correct/desired?

    #[test]
    fn column_breaks_basic() {
        let text = Rope::from("every wordthing should getits own");
        // TODO(minor): This width is in bytes
        let result = debug_breaks(&text, FONT_SIZE, 8.0);
        assert_eq!(
            result,
    ...
    fn soft_before_hard() {
        let text: Rope =
            "create abreak between THESE TWO\nwords andbreakcorrectlyhere\nplz"
                .into();
        let result = debug_breaks(&text, FONT_SIZE, 4.0);
        assert_eq!(
            result,
            vec![
                "create ",
    ...
    fn column_breaks_hard_soft() {
        ...
        assert_eq!(
            result,
            vec!["so ", "every ", "wordthing ", "should ", "getits ", "own",]
        );
    }
    ...
    fn dont_break_til_i_tell_you() {
        ...
        let result = debug_breaks(&text, FONT_SIZE, 12.0);
    ...
    fn newlines() {
        let text: Rope = "\n\n".into();
        let result = debug_breaks(&text, FONT_SIZE, 4.0);
        assert_eq!(result, Vec::<Cow<str>>::new());
    }
