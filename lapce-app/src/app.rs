...
impl AppData {
    ...
    fn app_view(&self, window_id: WindowId, info: WindowInfo) -> impl View {
        ...
        .style(move |base| {
            base.flex_col().size_pct(100.0, 100.0).scroll_bar_color(
                *window_data
                    .config
                    .get()
                    ...
                    container(|| {
                        svg(move || info.with(|info| info.icon.clone())).style(
                            move |base| {
                                ...
                                base.size_px(size, size).apply_opt(
        ...
        stack(|| {
            (
                container(child_view)
                    ...
                    .style(|base| {
                        base.align_items(Some(AlignItems::Center)).height_pct(100.0)
                    }),
                container(|| {
                ...
                .style(|base| {
                    base.absolute().padding_horiz_px(3.0).size_pct(100.0, 100.0)
                }),
                empty().style(move |base| {
                    let i = i.get();
                ...
                (
                    ...
                    stack(|| {
                        (
                            ...
                            .style(|base| {
                                base.margin_horiz_px(6.0).margin_vert_px(7.0)
                            }),
                            clickable_icon(
                                || LapceIcons::TAB_NEXT,
                                move || {
                            ...
                            .style(|base| base.margin_right_px(6.0)),
                        )
                    })
                    .on_resize(move |rect| {
                        size.set(rect.size());
                    })
                    .style(move |base| base.items_center()),
                )
            }),
            container(|| {
                scroll(|| {
                ...
                .style(|base| {
                    base.position(Position::Absolute)
                        .height_pct(100.0)
                        .max_width_pct(100.0)
                })
            })
            .style(|base| base.height_pct(100.0).flex_grow(1.0).flex_basis_px(0.0)),
            stack(|| {
                let size = create_rw_signal(Size::ZERO);
                (
                    clip(|| {
                    ...
                    .style(|base| base.items_center().height_pct(100.0)),
                )
            })
            .style(|base| base.height_pct(100.0)),
        )
...
fn editor_tab_content(
    ...
    let view_fn = move |child| {
        ...
        child.style(|base| base.size_pct(100.0, 100.0))
    };
    let active = move || editor_tab.with(|t| t.active);

    tab(active, items, key, view_fn).style(|base| base.size_pct(100.0, 100.0))
}
...
fn split_border(
    ...
    .style(|base| base.position(Position::Absolute).size_pct(100.0, 100.0))
}
...
fn split_list(
    ...
    let view_fn = move |(_index, content), main_split: MainSplitData| {
        ...
        child
            ...
            .style(move |base| {
                base.flex_grow(1.0).flex_basis(Dimension::Points(1.0))
            })
    };
    container_box(move || {
        Box::new(
            stack(move || {
                (
                    ...
                    .style(move |base| {
                        base.flex_direction(match direction() {
                            SplitDirection::Vertical => FlexDirection::Row,
                            SplitDirection::Horizontal => FlexDirection::Column,
                        })
...
fn main_split(window_tab_data: Arc<WindowTabData>) -> impl View {
    ...
    .style(move |base| {
        ...
        base.border_color(*config.get_color(LapceColor::LAPCE_BORDER))
                ...
                Box::new(
                    stack(move || {
                        (
                            ...
                            .style(|base| {
                                base.margin_right_px(6.0).max_width_pct(100.0)
                            }),
                            focus_text(
                                move || folder.clone(),
                    ...
                    .style(|base| {
                        base.align_items(Some(AlignItems::Center))
                            .max_width_pct(100.0)
                    }),
                )
            })
        ...
        PaletteItemContent::Command { .. } => {
            ...
            container_box(move || {
                Box::new(
                        ...
                        (
                            ...
                            stack(|| {
                                (list(
                                    ...
                                    |k| k.clone(),
                                    move |key| {
                                        label(move || key.clone()).style(
                                            move |base| {
                                                base.padding_horiz_px(5.0)
                                                    .padding_vert_px(1.0)
                                                    .margin_right_px(5.0)
                                                    .border(1.0)
                                                    .border_radius(6.0)
                    ...
                    .style(|base| base.width_pct(100.0).items_center()),
                )
            })
        }
...
fn palette_input(window_tab_data: Arc<WindowTabData>) -> impl View {
    ...
    container(move || {
        container(move || {
            text_input(editor, is_focused).style(|base| base.width_pct(100.0))
        })
        .style(move |base| {
            let config = config.get();
            base.width_pct(100.0)
    ...
    .style(|base| base.padding_bottom_px(5.0))
}
...
fn palette_content(
    ...
    stack(move || {
        (
            scroll(move || {
                ...
                virtual_list(
                    ...
                    move |(i, item)| {
                        ...
                        .style(|base| {
                            base.width_pct(100.0).cursor(CursorStyle::Pointer)
                        })
                        .hover_style(move |base| {
                            base.background(
                                *config
                ...
                .style(|base| base.width_pct(100.0).flex_col())
            })
            .on_ensure_visible(move || {
            ...
            .style(move |base| base.width_pct(100.0).min_height_px(0.0)),
            label(|| "No matching results".to_string()).style(move |base| {
                base.display(if items.with(|items| items.is_empty()) {
                    Display::Flex
                } else {
                    Display::None
                })
...
fn palette_preview(palette_data: PaletteData) -> impl View {
    ...
    .style(move |base| {
        base.display(if has_preview.get() {
            Display::Flex
        } else {
            Display::None
        })
...
fn palette(window_tab_data: Arc<WindowTabData>) -> impl View {
    ...
    .style(move |base| {
        base.display(if status.get() == PaletteStatus::Inactive {
            Display::None
        } else {
            Display::Flex
        })
...
fn hover(window_tab_data: Arc<WindowTabData>) -> impl View {
    ...
    scroll(|| {
                ...
                MarkdownContent::Text(text_layout) => container_box(|| {
                    Box::new(
                        rich_text(move || text_layout.clone())
                            .style(|base| base.max_width_px(600.0)),
                    )
                })
                .style(|base| base.max_width_pct(100.0)),
                MarkdownContent::Image { .. } => container_box(|| Box::new(empty())),
                MarkdownContent::Separator => container_box(|| {
                    Box::new(empty().style(move |base| {
                        base.width_pct(100.0)
                            .margin_vert_px(5.0)
                            .height_px(1.0)
                            .background(
        ...
        .style(|base| base.flex_col().padding_horiz_px(10.0).padding_vert_px(5.0))
    })
    .on_resize(move |rect| {
        layout_rect.set(rect);
    })
            ...
            move |(i, item)| {
                stack(|| {
                    (
                        ...
                        .style(move |base| {
                            let config = config.get();
                            base.width_px(config.editor.line_height() as f32)
                                .height_pct(100.0)
                                .align_items(Some(AlignItems::Center))
...
fn code_action(window_tab_data: Arc<WindowTabData>) -> impl View {
    ...
    scroll(move || {
                ...
                move |(i, item)| {
                    container(move || {
                        label(move || item.title().replace('\n', " "))
                            .style(|base| base.text_ellipsis().min_width_px(0.0))
                    })
                    .style(move |base| {
                        let config = config.get();
                        base.padding_horiz_px(10.0)
                            .align_items(Some(AlignItems::Center))
                            .min_width_px(0.0)
                            .width_pct(100.0)
                            .line_height(1.6)
        ...
        .style(|base| base.width_pct(100.0).padding_vert_px(4.0))
    })
    .on_ensure_visible(move || {
        let config = config.get();
        let active = active.get();
    ...
    container(|| {
        container(move || {
            text_input(editor, move || active.get())
                .style(|base| base.width_px(150.0))
        })
        .style(move |base| {
            let config = config.get();
            base.font_family(config.editor.font_family.clone())
                .font_size(config.editor.font_size() as f32)
                .border(1.0)
                .border_radius(6.0)
                .border_color(*config.get_color(LapceColor::LAPCE_BORDER))
...
fn workspace_tab_header(window_data: WindowData) -> impl View {
    ...
    let view_fn = move |(index, tab): (RwSignal<usize>, Arc<WindowTabData>)| {
        ...
        stack(|| {
            (
                container(|| {
                    ...
                    .style(move |base| base.size_pct(100.0, 100.0).items_center())
                })
                .draggable()
                .on_event(EventListener::DragStart, move |_| {
        ...
        .style(move |base| base.height_pct(100.0).width_px(tab_width.get() as f32))
    };
    ...
    stack(|| {
        (
            ...
            handle_titlebar_area(empty).style(|base| {
                base.height_pct(100.0).flex_basis_px(0.0).flex_grow(1.0)
            }),
        )
    })
    .on_resize(move |rect| {
