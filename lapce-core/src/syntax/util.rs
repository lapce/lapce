...
pub fn matching_bracket_special(c: char) -> Option<char> {
    Some(match c {
        '{' => '}',
        '}' => '{',
        '(' => ')',
    ...
    for c in line.chars().rev() {
        if let Some(left) = matching_pair_direction(c) {
            let key = if left {
                ...
                matching_bracket_special(c).unwrap()
            };
...
pub fn str_matching_pair<R: ToStaticTextType>(c: &str) -> Option<R>
where
    &'static str: ToStaticTextType<R>,
{
    let mut chars = c.chars();
    match (chars.next(), chars.next()) {
        (Some(c), None) => matching_bracket_general(c),
        _ => None,
    }
}
