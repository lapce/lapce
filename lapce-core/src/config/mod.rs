...
pub const LOGO: &str = include_str!(concat!(
...
const DEFAULT_LIGHT_THEME: &str = include_str!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../defaults/light-theme.toml"
));
const DEFAULT_DARK_THEME: &str = include_str!(concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../defaults/dark-theme.toml"
));
const DEFAULT_ICON_THEME: &str = include_str!(concat!(
    env!("CARGO_MANIFEST_DIR"),
...
pub fn find_all_volts() -> Vec<lapce_rpc::plugin::VoltMetadata> {
...
pub fn load_volt(path: &Path) -> Result<lapce_rpc::plugin::VoltMetadata> {
    let mut file = std::fs::File::open(path)?;
    let mut contents = String::new();
    file.read_to_string(&mut contents)?;
    let mut meta: lapce_rpc::plugin::VoltMetadata = toml::from_str(&contents)?;
